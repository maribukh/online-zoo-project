...
export function initDonation() {
    ...
    const selectCurrent = document.querySelector('#step1 .select-current');
    const selectList = document.getElementById('selectList');
    const selectBox = document.getElementById('selectBox');
    const otherInput = document.querySelector('.other-input');
    const amountBtns = document.querySelectorAll('#step1 .amount-btn');
    ...
    function openStepModal(amount) {
        ...
        if (amount && amount > 0) {
            ...
            const matchedBtn = Array.from(amountBtns).find((b) => parseInt(b.textContent?.replace('$', '') ?? '0', 10) === amount);
    ...
    donationModal
        ?.querySelectorAll('.donate-btn:not(.donate-btn_other)')
        .forEach((btn) => {
    ...
    async function loadPetsIntoDropdown() {
        if (!selectList)
            return;
        try {
    ...
    function checkStep2Valid() {
        ...
        if (toStep3Btn)
            toStep3Btn.disabled = !(nameOk && emailOk);
    }
    nameInput?.addEventListener('input', () => {
        const ok = nameRegex.test(nameInput.value);
        nameInput.parentElement?.classList.toggle('error', nameInput.value.length > 0 && !ok);
    ...
    emailInput?.addEventListener('input', () => {
        ...
        emailInput.parentElement?.classList.toggle('error', emailInput.value.length > 0 && !ok);
        const err = emailInput.parentElement?.querySelector('.error-message');
        if (err)
            err.textContent = ok ? '' : 'Enter a valid email address';
        checkStep2Valid();
    });
    nameInput?.addEventListener('focus', () => nameInput.parentElement?.classList.remove('error'));
    emailInput?.addEventListener('focus', () => emailInput.parentElement?.classList.remove('error'));
    function isValidExpiry() {
        if (!selectedMonth || !selectedYear)
            return false;
        const now = new Date();
        const exp = new Date(parseInt(selectedYear, 10), parseInt(selectedMonth, 10) - 1, 1);
    ...
    cardInput?.addEventListener('input', () => {
        ...
        cardInput.parentElement?.classList.toggle('error', cardInput.value.length > 0 && !ok);
        checkStep3Valid();
    });
    cvvInput?.addEventListener('input', () => {
        cvvInput.value = cvvInput.value.replace(/\D/g, '').slice(0, 3);
        const ok = /^\d{3}$/.test(cvvInput.value);
        cvvInput.parentElement?.classList.toggle('error', cvvInput.value.length > 0 && !ok);
        checkStep3Valid();
    });
    function setupDateSelect(boxId, listId, type) {
        ...
        list?.querySelectorAll('.select-item').forEach((item) => {
            item.addEventListener('click', () => {
                ...
                const current = box?.querySelector('.select-current');
                if (current) {
    ...
    function renderSaveCardSection() {
        ...
        <ul class="select-list select-hide" id="savedCardsList">
          ${savedCards.map((c, i) => `<li class="select-item" data-index="${i}">${maskCard(c.cardNum)}</li>`).join('')}
            ...
            const box = wrap.querySelector('#savedCardsBox');
            const list = wrap.querySelector('#savedCardsList');
            box?.addEventListener('click', (e) => {
                e.stopPropagation();
                list?.classList.toggle('select-hide');
            ...
            list?.querySelectorAll('.select-item').forEach((item) => {
                item.addEventListener('click', () => {
                    ...
                    if (!card)
                        return;
                    if (cardInput)
                        cardInput.value = card.cardNum;
                    if (cvvInput)
                        cvvInput.value = card.cvv;
                    selectedMonth = card.expMonth;
                    selectedYear = card.expYear;
                    const mCur = document.querySelector('#monthBox .select-current');
                    const yCur = document.querySelector('#yearBox .select-current');
                    if (mCur) {
                    ...
                    const cur = box?.querySelector('.select-current');
                    if (cur) {
        ...
        if (footer)
            step3.insertBefore(section, footer);
    }
    ...
    completeBtn?.addEventListener('click', async () => {
        ...
        try {
            ...
            if (res.ok) {
                showNotification(`Thank you for your donation of $${selectedAmount} to ${selectedPetName}!`);
            }
            else {
                const err = (await res.json());
                showNotification(err.error ?? 'Something went wrong. Please, try again later.', true);
            }
        }
        catch {
    ...
    document.addEventListener('click', () => {
        ...
        document
            .querySelectorAll('#stepModal .custom-select-container, #donationModal .custom-select-container')
