const API_URL = 'https://vsqsnqnxkh.execute-api.eu-central-1.amazonaws.com/prod';
const elements = {
...
const rules = {
    ...
    login: (v) => /^[a-zA-Z][a-zA-Z]{2,}$/.test(v),
    password: (v) => v.length >= 6 && /[^a-zA-Z0-9]/.test(v),
    confirm: (v) => v === elements.inputs.password.value && v.length > 0,
};
const state = {
    name: false,
    email: false,
...
function updateUI(name, isValid, message) {
...
Object.keys(elements.inputs).forEach((key) => {
    ...
    input.addEventListener('blur', () => {
        ...
        let msg = '';
        if (name === 'name')
            msg = 'Min 3 letters, only English letters.';
        if (name === 'email')
            msg = 'Enter a valid email address.';
        if (name === 'login')
            msg = 'Min 3 chars, must start with a letter.';
        if (name === 'password')
            msg = 'Min 6 chars + 1 special character.';
        if (name === 'confirm')
...
function setupToggle(btnId, inputId, offId, onId) {
    document.getElementById(btnId)?.addEventListener('click', () => {
        const input = document.getElementById(inputId);
        const isHidden = input.type === 'password';
        input.type = isHidden ? 'text' : 'password';
        document.getElementById(offId).style.display = isHidden
            ? 'none'
            : 'block';
        document.getElementById(onId).style.display = isHidden
            ? 'block'
            : 'none';
    });
}
...
elements.form.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (res.status === 409) {
            ...
            showError(data.message ?? 'Invalid data. Please check your input and try again.');
        }
