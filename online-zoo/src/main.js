...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    function renderFeedbacks(feedbacks, container) {
        ...
        for (let i = 0; i < feedbacks.length; i += cardsPerPage) {
            ...
            chunk.forEach((fb) => {
                ...
                const fbRecord = fb;
                const dateString = fbRecord['month'] && fbRecord['year']
    ...
    function initTestimonialsSlider(grid) {
        const btnPrev = document.querySelector('.testimonials-nav button:first-child');
        const btnNext = document.querySelector('.testimonials-nav button:last-child');
        const tDots = document.querySelectorAll('.testimonials-pagination .dot');
        let currentIndex = 0;
        function getAllCards() {
            return Array.from(grid.querySelectorAll('.testimonial-card'));
        }
        function isMobile() {
        ...
        const update = (index) => {
            ...
            tDots[dotIndex]?.classList.add('active');
            currentIndex = target;
        };
    ...
    document
        .querySelectorAll('.btn-orange:not(#heroViewBtn), .btn-outline-white, .btn-fav, .feed-link, .btn-box')
    ...
    document
        .querySelectorAll('.modal-close, #closeModal, #closeStepModal, #closeBurger')
    ...
    document.querySelectorAll('[data-hover-src]').forEach((btn) => {
        ...
        if (icon) {
            const def = icon.src;
            btn.addEventListener('mouseenter', () => (icon.src = btn.dataset['hoverSrc'] ?? def));
